...
/// <summary>
/// https://github.com/Qkyyy/PickupablesObjects-UNITY/blob/master/Assets/Scripts/PickupableObject.cs
/// </summary>
...
public class PickableObject : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        playerController = GameObject.Find("Character").GetComponent<HeroMovement>();
    }
    ...
    void HandleObjectRotation()
    {
        if (pickedUp == true)
        {
            if (Input.GetKey(KeyCode.R))
            {
                ...
                float rotationX = Input.GetAxis("Mouse X") * 8f;
                float rotationY = Input.GetAxis("Mouse Y") * 8f;

                transform.Rotate(Camera.main.transform.up, -Mathf.Deg2Rad * rotationX);
                transform.Rotate(Camera.main.transform.right, Mathf.Deg2Rad * rotationY);
            }
